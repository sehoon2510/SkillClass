...
class History 
{
    ...
    get yearList() {
        return Array.from(
            new Set(
                this.historys.map(function(value, index) {
                    return value.year;
                })
            )
        )
    ...
    drow() {
        ...
        this.historys.filter(value => value.year == this.active );

        this.historys.filter(value => value.year == this.active ).forEach(history => {
            this.content.innerHTML += `
        ...
        this.yearList.forEach(year => {
            this.Tabs.innerHTML += `<div class="tab__item ${this.active == year ? 'active' : ''}" data-year="${year}">${year}년</div>`;
        });
    ...
    StorageLoad() {
        ...
        if(!this.active) {
            this.active = this.historys.reduce(function(pvalue, cvalue) {
                if(pvalue < cvalue.year) pvalue = cvalue.year;
                return pvalue;
            }, 0);
        }
